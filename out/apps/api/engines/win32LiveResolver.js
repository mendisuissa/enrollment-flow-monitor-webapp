...
function expandQueries(query, mode) {
...
async function searchWingetCatalog(query) {
...
function extractCommandsFromHtml(html) {
    const blocks = [
...
async function searchSilentInstallHq(query) {
    ...
    const urls = [...html.matchAll(/href=["'](https:\/\/silentinstallhq\.com\/[^"'#]+)["']/gi)]
        .map((match) => match[1])
        .filter((url) => Boolean(url) && !url.includes('/?s='));
    ...
    for (const url of uniqueUrls) {
        try {
            ...
            if (!commands.installCommand && !commands.uninstallCommand)
                continue;
            rows.push({
                title,
                url,
...
function buildDetectScript(appName) {
...
function scoreResult(query, candidateName, publisher, sourceType, hasBothCommands) {
    ...
    for (const token of qTokens) {
        if (cTokens.includes(token))
            score += 30;
        if (normalize(publisher).includes(token))
            score += 10;
    }
...
export async function resolveWin32Search(query, mode) {
    const trimmed = query.trim();
    if (!trimmed) {
        return {
            query: '',
    ...
    try {
        ...
        for (const q of queries) {
            ...
            for (const row of rows) {
                if (candidates.some((item) => item.packageId === row.packageIdentifier))
                    continue;
                candidates.push({
    ...
    try {
        ...
        for (const q of queries) {
            ...
            for (const row of rows) {
                ...
                if (!row.installCommand && !row.uninstallCommand)
                    continue;
                if (candidates.some((item) => item.sourceUrl === row.url))
                    continue;
                candidates.push({
...
//# sourceMappingURL=win32LiveResolver.js.map
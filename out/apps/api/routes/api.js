...
function ensureAdmin(req) {
    const upn = String(req.session?.account?.username ?? '').toLowerCase().trim();
...
function getSessionTenantId(req) {
    const tenantId = String(req.session?.account?.tenantId ?? '').trim();
    return tenantId || null;
}
function ensureTenantBound(req) {
    if (config.mockMode)
        return;
...
async function assertKnownDeviceId(req, deviceId) {
...
async function assertKnownIncidentSignature(req, signature) {
    const data = await getScopedData(req);
...
async function mergeIncidentWorkflows(rows) {
...
function buildReportData(data, upn, tenantId) {
...
async function graphPostAction(accessToken, path, body) {
...
apiRouter.post('/incidents/:signature/workflow', async (req, res) => {
    ...
    if (!signature)
        return res.status(400).json({ message: 'Missing incident signature.' });
...
Last Sync: ${d.lastSyncDateTime}`
        ...
        const isTokenError = (error instanceof Error && (error.name === 'TokenExpiredError' ||
...
apiRouter.get('/graph/compliance-drift', async (req, res) => {
    try {
        ...
        let allDevices = [];
        let url = G + '/v1.0/deviceManagement/managedDevices?$select=id,complianceState&$top=999';
        while (url) {

...
export interface IncidentRepository {
  ...
  upsertWorkflow(input: { signature: string; owner: string; status: IncidentWorkflowStatus; notes: string }): Promise<IncidentWorkflowRecord>;
}


interface PersistedWorkflowRow {
...
export class PrismaIncidentRepository implements IncidentRepository {
  ...
  async listWorkflows(): Promise<IncidentWorkflowRecord[]> {
    const workflows = await prisma.incidentWorkflow.findMany({
      orderBy: [{ updatedAt: 'desc' }]
    });

    return workflows.map((workflow: PersistedWorkflowRow) => ({
      signature: workflow.incidentSignature,
      owner: workflow.owner,
      status: workflow.status as IncidentWorkflowStatus,
      notes: workflow.notes,
      updatedAt: workflow.updatedAt.toISOString()
    }));
  }

  async upsertWorkflow(input: { signature: string; owner: string; status: IncidentWorkflowStatus; notes: string }): Promise<IncidentWorkflowRecord> {

...
// ── Build incidents from troubleshootingEvents (enrollment failures) ──────────
...
export function buildEnrollmentIncidents(failures: EnrollmentFailureRow[], thresholds: { Low: number; Medium: number; High: number }): IncidentRow[] {
  if (!failures || failures.length === 0) return [];

  const grouped = new Map<string, EnrollmentFailureRow[]>();
  ...
  for (const [key, rows] of grouped.entries()) {
    ...
    const sorted = rows.slice().sort((a, b) =>
      (a.failureDateTime ?? '').localeCompare(b.failureDateTime ?? '')
    );
    const first = sorted[0];
    const last = sorted[sorted.length - 1];

    const count = rows.length;
    const severity: IncidentRow['severity'] =
      count >= thresholds.High ? 'High' :

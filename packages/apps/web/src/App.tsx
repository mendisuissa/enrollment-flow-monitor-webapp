  ...
  return (
    <div className="app-shell">
      ...
      <div className={isMobile ? "flex flex-col gap-3" : "content-grid"}>
            ...
            <div className="fix-queue-shell">
              ...
              <div className="fix-queue-list">
                {filteredRows.map((row, index) => {
                  ...
                  return (
                    <button key={String(row['id'] ?? index)} className={`fix-card ${selected ? 'active' : ''}`} onClick={() => setSelectedIndex(index)}>
                      <div className="fix-card-top">
                        <div className="fix-card-title-wrap">
                          ...
                          <span className={`severity-pill ${severity}`}>{String(row['severity'] ?? 'Low')}</span>
                      ...
                      <div className="fix-meta-grid">
                        <div><span>Category</span><strong>{toText(row['normalizedCategory'])}</strong></div>
                        <div><span>Owner</span><strong>{toText(row['owner'])}</strong></div>
                        <div><span>Status</span><strong>{toText(row['status'])}</strong></div>
                        <div><span>Confidence</span><strong>{Math.round(Number(row['rootCauseConfidence'] ?? 0) * 100)}%</strong></div>
                      </div>
        ...
        <div className={`panel detail-rail ${currentView === 'dashboard' ? 'dashboard-rail' : ''} ${!detailsText ? 'is-empty' : ''}`}>
          ...
          {currentView === 'incidents' && selectedIncident && !selectedIncident['isPlaceholder'] && (
            <div className="incident-workflow-card">
              ...
              <div className="incident-workflow-meta">
                Last updated: <span>{toText(selectedIncident['workflowUpdatedAt'] ?? 'Not saved yet')}</span>
              </div>
              <button className="btn btn-primary incident-workflow-save" onClick={onSaveIncidentWorkflow} disabled={incidentWorkflowSaving}>
